...
class Parser:
    ...
    def _can_start_composite_literal(self):
        ...
        if getattr(self, "_no_composite_lit", False):
            # Do not consume a control-flow body as an empty composite literal.
            # `last.next {}` is the particularly important case: the `{` is
            # the for/if/switch body, even though it is followed immediately
            # by `}`.  Preserve the useful `T{}` exception only when the token
            # immediately before the brace looks like a type name.
            if self.peek("{") and self.i + 1 < len(self.tokens):
                nxt = self.tokens[self.i + 1]
                prev = self.tokens[self.i - 1].value if self.i else ""
                if nxt.value == "}" and isinstance(prev, str) and prev[:1].isupper():
                    return True
            return False
        ...
        if self.peek("{") and self.i > 0:
            ...
            if (prev_token.kind in ("ident", "keyword") and
                    isinstance(prev_token.value, str) and
                    prev_token.value[:1].isupper()):
                return True
        if self.i <= 1:
            return True
        ...
        if t.kind in ("ident","keyword"):
            ...
            if self.peek("["):
                ...
                if depth == 0 and after in ("(", "{", "."):
                    ...
                    if after == "(" or (after == "{" and (looks_type or t.value.endswith("Var") or not getattr(self, "_no_composite_lit", False))) or (after == "." and looks_type):
                        generic_instantiated = True
                        while self.i <= j:
                            self.take()
            ...
            if self.peek("["):
                ...
                if depth == 0 and ((after == "(" and type_arg_like) or (after == "{" and type_arg_like) or (after == "." and type_arg_like)):
                    while self.i <= j:
                        self.take()
                    generic_instantiated = True
