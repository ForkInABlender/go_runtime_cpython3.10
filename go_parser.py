...
class Parser:
    ...
    def parse_type_decl(self):
        ...
        if self.maybe("("):
            ...
            while not self.peek(")"):
                n=self.take().value
                self.maybe("=")
                ty=self.parse_type(); specs.append(TypeSpec(line,n,ty)); self.semi()
    ...
    def parse_param_group(self):
        ...
        while not self.peek(")"):
            ...
            if self.t.kind in ("ident", "keyword"):
                ...
                # `chan<- T` is itself a type, not a parameter named `chan`.
                if self.t.value == "chan" and n0 == "<-":
                    named = False
                if n0 == "...":
                    named = True
    ...
    def _bracket_starts_array_field(self) -> bool:
        """Return True when `name [N]T` is a named array field.

        Generic embedded fields such as `nistCurve[*nistec.P256Point]` also
        begin with `[`, so looking only inside the brackets is ambiguous.  The
        token after the matching `]` resolves the common cases: an array field
        is followed by its element type, while an embedded generic type is
        followed by a field boundary or tag.
        """
        if self.i + 1 >= len(self.tokens) or self.tokens[self.i + 1].value != "[":
            return False
        depth = 0
        j = self.i + 1
        while j < len(self.tokens):
            ...
            if v == "[":
                ...
                if depth == 0:
                    break
            j += 1
        if depth != 0 or j + 1 >= len(self.tokens):
            return False
        after = self.tokens[j + 1]
        if after.value in (";", "}") or after.kind == "string":
            return False
        return after.kind in ("ident", "keyword") or after.value in ("*", "[", "map", "chan", "func", "interface", "struct")
        ...
        if self.maybe("struct"):
            ...
            while not self.peek("}"):
                ...
                if (self.t.kind in ("ident","keyword")
                        ...
                        and self.tokens[self.i+1].kind not in ("eof", "string")
                        and not (self.tokens[self.i+1].value == "[" and not self._bracket_starts_array_field())):
                    names=self.parse_ident_list()
    ...
    def _can_start_composite_literal(self):
        ...
        if getattr(self, "_no_composite_lit", False):
            # Control-flow conditions are unambiguously expression context;
            # a following `{` is the statement body, not a composite literal.
            # Composite literals inside the condition itself are parsed with
            # an explicitly cleared flag by the relevant parenthesized/index
            # parsing paths.
            return False
        if getattr(self, "_allow_composite", False):
            return True
        # A type name immediately followed by `{` is a composite literal,
        # including after a selector (`pkg.Type{...}`).  Looking two tokens
        # back is insufficient here because the immediate predecessor is the
        # type name itself.  Control-flow bodies are parsed with
        # _no_composite_lit, so this does not steal `if Foo { ... }`.
        if self.peek("{") and self.i > 0:
            prev_token = self.tokens[self.i - 1]
            if prev_token.kind in ("ident", "keyword"):
                # In ordinary expression context, `T{...}` is a composite
                # literal regardless of whether the type name is exported.
                # Control-flow conditions suppress this branch above, which
                # prevents `if x {` / `switch x {` from being misread.
                return True
        if self.i <= 1:
            return True
    ...
    def parse_primary(self):
        ...
        if t.value == "func":
            ...
            results=[]
            if self.peek("("):
                results=self.parse_param_group()
            elif not self.peek("{") and not self.peek(")") and not self.peek(";") and not self.peek("}") and not self.peek(":"):
                results=[self.parse_type()]
            # `func(...) { ... }` is a function literal; `func(...)` and
            # `func(...) T` are function types (notably in type-switch cases).
            if self.peek("{"):
                body=self.parse_block()
                return self.parse_postfix(Expr(line,"func_lit",None,[params,results,body]))
            return self.parse_postfix(Expr(line,"func_type",None,[params,results]))
        if t.kind in ("ident","keyword"):
            ...
            if self.peek("["):
                ...
                if depth == 0 and after in ("(", "{", "."):
                    ...
                    bracket_tokens = self.tokens[self.i:j+1]
                    bracket_text = [tok.value for tok in bracket_tokens]
                    looks_type = bool(t.value[:1].isupper()) and ":" not in bracket_text
                    has_literal = any(tok.kind in ("string", "number", "rune") for tok in bracket_tokens)
                    type_syntax = (not has_literal and any(
                        v in ("*", "~", "interface", "struct", "map", "chan", "func")
                        for v in bracket_text
                    ))
                    if (after == "(" or
                            (after == "{" and (not getattr(self, "_no_composite_lit", False) or
                                               (type_syntax and not getattr(self, "_no_composite_lit", False)))) or
                            (after == "." and looks_type)):
                        generic_instantiated = True
                        while self.i <= j:
                            self.take()
            ...
            # Generic instantiation may follow a qualified identifier, e.g.
            # `concurrent.NewHashTrieMap[*abi.Type, any]()`.  The selector
            # chain above is parsed before the bracket list, so handle that
            ...
            if self.peek("["):
                ...
                type_arg_like = (
                    ...
                    and (
                        qualified_type
                        or any(v in ("*", "~", "interface", "struct", "map", "chan", "func") for v in bracket_text)
                    )
                )
                if depth == 0 and (
                        (after == "(" and type_arg_like) or
                        (after == "{" and type_arg_like and (
                            not getattr(self, "_no_composite_lit", False) or
                            any(v in ("*", "~", "interface", "struct", "map", "chan", "func") for v in bracket_text)
                        )) or
                        (after == "." and type_arg_like)):
                    while self.i <= j:
                        self.take()
                    generic_instantiated = True
            ...
            if self.peek("{") and t.value not in KEYWORDS and (allow or qualified_ok or generic_instantiated):
                ...
                self.take(); elems=[]
                # Once the opening brace is consumed, nested literals are
                # unambiguously expression context even when the enclosing
                # expression was parsed with _no_composite_lit.
                prev_nc_literal = getattr(self, "_no_composite_lit", False)
                self._no_composite_lit = False
                try:
                    while not self.peek("}"):
                        first=self.parse_expr()
                    ...
                    self._no_composite_lit = prev_nc_literal
