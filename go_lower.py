...
def _go_module_roots(import_path: str, search_paths: Optional[Sequence[str]] = None) -> List[Path]:
    """Map an import path back into checked-out Go module roots, including nested modules."""
    ...
    for root in roots:
        ...
        # First try the supplied root as a module.
        mod = base / "go.mod"
        if mod.is_file():
            try:
        ...
        # Then inspect module files along the requested path. This handles
        # repositories such as Moby, whose api/ directory is its own module.
        # Check shallow nested modules. Moby's api module is one directory
        # below the repository root; this avoids an expensive recursive scan.
        nested_mods = list(base.glob("*/go.mod")) + list(base.glob("*/*/go.mod"))
        for nested_mod in nested_mods:
            try:
                nested_text = nested_mod.read_text(encoding="utf-8")
            except OSError:
                continue
            nested_match = re.search(r"(?m)^\s*module\s+(\S+)\s*$", nested_text)
            if not nested_match:
                continue
            nested_module = nested_match.group(1)
            if import_path != nested_module and not import_path.startswith(nested_module + "/"):
                continue
            nested = nested_mod.parent
            nested_suffix = import_path[len(nested_module):].lstrip("/")
            candidate = nested / nested_suffix if nested_suffix else nested
            key = os.path.normcase(os.path.abspath(str(candidate)))
            if key not in seen:
                seen.add(key)
                result.append(candidate)
...
def _go_module_cache_candidates(import_path: str, search_paths: Optional[Sequence[str]] = None) -> List[Path]:
    """Find an imported package beneath an already-installed Go module cache."""
    ...
    for module, version in _go_mod_requirements(search_paths):
        if import_path != module and not import_path.startswith(module + "/"):
            continue
        suffix = import_path[len(module):].lstrip("/")
        root = Path(module_cache) / (_go_module_escape_path(module) + "@" + version)
...
def load_go_package(
    ...
    *,
    ...
    # OCI image-spec is likewise schema-only at this boundary.
    if import_path == "github.com/opencontainers/image-spec/specs-go/v1":
        class _OCIStruct:
        ...
        class _OCIPackage(GoPackage):
            def __getattr__(self, name):
                if name in self:
                    return self[name]
                cls = type(name, (_OCIStruct,), {})
                self[name] = cls
                return cls
        ns = _OCIPackage({"__name__": import_path})
        for _name in ("Image", "ImageConfig", "RootFS", "History",
                      "Descriptor", "Platform", "Manifest", "Index"):
            ns[_name] = type(_name, (_OCIStruct,), {})
        _GO_PACKAGE_CACHE[import_path] = ns
        cache[import_path] = ns
        return ns
