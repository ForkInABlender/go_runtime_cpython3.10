...
class Lowerer:
    def __init__(
        ...
        self._method_bindings: List[tuple[str, str]] = []
    ...
    def lower(self, tree: File) -> str:
        ...
        for receiver_type, method_name in self._method_bindings:
            self.emit(f"{py_ident(receiver_type)}.{py_ident(method_name)} = {py_ident(method_name)}")
        return "\n".join(self.lines) + "\n"
    ...
    def lower_func(self, d: FuncDecl) -> None:
        ...
        if receiver:
            ...
            if receiver_type:
                self._method_bindings.append((receiver_type, d.name))
...
def load_go_package(
    ...
    *,
    ...
    # Small host-backed shims for foundational standard-library packages.
    # Translating the entire Go standard library just to satisfy `import "fmt"`
    # pulls in runtime/reflect and exposes unrelated parser limitations.  These
    # shims keep source-level imports working while the Go lowering layer grows.
    if import_path == "fmt":
        import builtins

        def _Print(*args):
            return builtins.print(*args, end="")

        def _Println(*args):
            return builtins.print(*args)

        def _Printf(format_string, *args):
            return builtins.print(str(format_string) % args, end="")
