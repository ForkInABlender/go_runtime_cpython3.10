...
def py_ident(name: str) -> str:
    ...
    if keyword.iskeyword(name):
        return f"go_{name}"
    # Import/package identifiers are normally already legal Go identifiers,
    # but unresolved external module paths can expose repository names such as
    # "specs-go".  Keep generated Python syntactically valid without altering
    # ordinary Go identifiers.
...
class Lowerer:
    ...
    def lower(self, tree: File) -> str:
        ...
        self.indent -= 1
        self.emit("def _go_make(type_or_zero=None, size=0, *extra):")
        self.indent += 1
        self.emit("if _go_builtins.isinstance(size, _go_builtins.int): return [None] * size")
        self.emit("return []")
        self.indent -= 1
        ...
        if any(isinstance(n, ForStmt) and n.range_expr is not None for n in self._walk(tree)):
            ...
            self.indent -= 1
            self.emit("def _go_range_keys(x):")
            self.indent += 1
            self.emit("if callable(x):")
            self.indent += 1
            self.emit("values = []")
            self.emit("x(lambda *v: (values.append(v), True)[1])")
            self.emit("return (v[0] if len(v) == 1 else tuple(v) for v in values)")
            self.indent -= 1
            self.emit("return x.keys() if isinstance(x, dict) else range(len(x))")
            self.indent -= 1
            self.emit()
        ...
        # Function declarations are available throughout a Go package before
        # package initialization runs, regardless of their textual position.
        # Emit them before variables/constants so initializers may call a
        # function declared later in the source.
        _func_decls = []
        _nonfunc_decls = []
        for _decl in other_decls:
            _items = _decl if isinstance(_decl, list) else [_decl]
            if isinstance(_decl, list):
                _func_decls.extend(d for d in _items if isinstance(d, FuncDecl))
                _rest = [d for d in _items if not isinstance(d, FuncDecl)]
                if _rest:
                    _nonfunc_decls.append(_rest)
            elif isinstance(_decl, FuncDecl):
                _func_decls.append(_decl)
            else:
                _nonfunc_decls.append(_decl)
        # Go permits package-level constants, variables, and functions to be
        # referenced independently of textual order.  In particular, a
        # function body is deferred until call time, but constant expressions
        # are evaluated while the package is being loaded.  Emit constants
        # before functions/variables so a later constant such as
        # time.wallToInternal exists before earlier constants such as maxWall
        # and minWall evaluate.  Keep grouped const declarations intact so
        # iota retains its Go semantics.
        _const_decls = []
        _var_decls = []
        for _decl in _nonfunc_decls:
            _items = _decl if isinstance(_decl, list) else [_decl]
            if isinstance(_decl, list):
                _const_items = [d for d in _items if isinstance(d, ConstSpec)]
                _var_items = [d for d in _items if isinstance(d, VarSpec)]
                if _const_items:
                    _const_decls.append(_const_items)
                _var_decls.extend(_var_items)
            elif isinstance(_decl, ConstSpec):
                _const_decls.append([_decl])
            elif isinstance(_decl, VarSpec):
                _var_decls.append(_decl)
            else:
                _var_decls.append(_decl)
        ...
        def _expr_names(_x):
            ...
            if isinstance(_x, list):
                out = set()
                for _v in _x:
                    out.update(_expr_names(_v))
                return out
            if isinstance(_x, Expr):
                out = {str(_x.value)} if _x.kind == "name" else set()
                for _c in _x.children:
                    out.update(_expr_names(_c))
                return out
            return set()

        # Go package-level constants are dependency ordered, including across
        # const blocks and within a single block.  A Python assignment cannot
        # refer to a later constant, so flatten the declarations into records,
        # preserve each declaration's original iota value, and materialize the
        # implicit (omitted) expressions before dependency sorting.
        _const_records = []
        _const_name_to_record = {}
        for _gi, _group in enumerate(_const_decls):
            _last_values = None
            for _ci, _cs in enumerate(_group):
                _effective_values = list(_cs.values) if _cs.values else list(_last_values or [])
                _record = (_cs, _gi, _ci, _effective_values)
                _const_records.append(_record)
                for _name in _cs.names:
                    _const_name_to_record[_name] = len(_const_records) - 1
                if _effective_values:
                    _last_values = _effective_values

        _const_deps = [set() for _ in _const_records]
        for _ri, (_cs, _gi, _ci, _values) in enumerate(_const_records):
            for _v in _values:
                for _name in _expr_names(_v):
                    _dep = _const_name_to_record.get(_name)
                    if _dep is not None and _dep != _ri:
                        _const_deps[_ri].add(_dep)

        _ordered_const_records = []
        _remaining_const = set(range(len(_const_records)))
        while _remaining_const:
            _ready = sorted(i for i in _remaining_const
                            if _const_deps[i].isdisjoint(_remaining_const))
            if not _ready:
                # A real Go constant cycle is invalid. Preserve source order so
                # the generated Python exposes the actual failure rather than
                # inventing a value.
                _ordered_const_records.extend(_const_records[i] for i in sorted(_remaining_const))
                break
            _ordered_const_records.extend(_const_records[i] for i in _ready)
            _remaining_const.difference_update(_ready)

        # Package-level variables, like constants, are initialized according
        # to their dependency graph rather than textual order.  This matters
        # for the standard library's time package: UTC depends on utcLoc,
        # while utcLoc is declared later in zoneinfo.go.  Predeclaring every
        # variable is not sufficient because it leaves the earlier assignment
        # with the value None.
        _var_records = []
        _var_name_to_record = {}
        for _vi, _vd in enumerate(_var_decls):
            _names = list(getattr(_vd, "names", []) or [])
            _values = list(getattr(_vd, "values", []) or [])
            _var_records.append((_vd, _vi, _values))
            for _name in _names:
                _var_name_to_record[_name] = len(_var_records) - 1

        _var_deps = [set() for _ in _var_records]
        for _ri, (_vd, _vi, _values) in enumerate(_var_records):
            for _value in _values:
                for _name in _expr_names(_value):
                    _dep = _var_name_to_record.get(_name)
                    if _dep is not None and _dep != _ri:
                        _var_deps[_ri].add(_dep)

        _ordered_var_records = []
        _remaining_vars = set(range(len(_var_records)))
        while _remaining_vars:
            _ready = sorted(i for i in _remaining_vars
                            if _var_deps[i].isdisjoint(_remaining_vars))
            if not _ready:
                # A genuine Go package-variable initialization cycle is invalid.
                # Preserve source order so the generated runtime exposes the
                # underlying failure rather than fabricating a value.
                _ordered_var_records.extend(_var_records[i] for i in sorted(_remaining_vars))
                break
            _ordered_var_records.extend(_var_records[i] for i in _ready)
            _remaining_vars.difference_update(_ready)
        _ordered_vars = [record[0] for record in _ordered_var_records]

        ordered_decls = type_decls + _func_decls + _ordered_vars
        self._top_level_decls = [d for decl in ordered_decls for d in (decl if isinstance(decl, list) else [decl])]
        # Go package-level variables are initialized according to dependency
        # order, not textual order.  Python executes assignments immediately,
        # so predeclare every package variable before evaluating any initializer.
        # This permits cases such as time.UTC = &utcLoc where utcLoc appears
        # later in the source file.
        _predeclared_vars = []
        for _decl in self._top_level_decls:
            if isinstance(_decl, VarSpec):
                _predeclared_vars.extend(_decl.names)
        for _name in _predeclared_vars:
            if _name != "_":
                self.emit(f"{py_ident(_name)} = None")
        if _predeclared_vars:
            self.emit()
        for _cs, _gi, _ci, _values in _ordered_const_records:
            self.iota = _ci
            self.lower_var(VarSpec(_cs.line, _cs.names, _cs.type_expr, _values))
        self.iota = 0
        self._last_const_values = None
    ...
    def _import_package_name(self, imp: Import) -> str:
        ...
        if declared:
            return py_ident(declared)
        # When an external module is intentionally unresolved in translation
        # mode, approximate the common Go convention for repository names
        # prefixed with "go-" (for example github.com/moby/go-archive has
        # package name "archive").  Keep the result a valid Python identifier.
        fallback = imp.path.rsplit("/", 1)[-1]
        if fallback.startswith("go-") and len(fallback) > 3:
            fallback = fallback[3:]
        elif fallback.endswith("-go") and len(fallback) > 3:
            fallback = fallback[:-3]
        return py_ident(fallback)
    ...
    def _walk(self, value):
        # Iterative AST traversal.  Generated Go data tables (notably
        # x/text/unicode/norm/tables*.go) can produce AST nesting deeper than
        # Python's recursion limit.  _scan_needs only needs visitation, so do
        # not recursively descend through the tree.
        stack = [value]
        while stack:
            cur = stack.pop()
    ...
    def lower_func(self, d: FuncDecl) -> None:
        ...
        if labels:
            ...
            # A small number of Go standard-library functions are compiler/runtime
            # intrinsics declared without a Go body.  Preserve their host
            # semantics where the translated package actually executes them.
            if d.name == "runtimeNano":
                self.emit("import time as _go_host_time")
                self.emit("return _go_host_time.monotonic_ns()")
            else:
                self.emit("pass")
        else:
        ...
        if receiver:
            ...
            if receiver_type:
                ...
                if (d.name not in _GO_PREDECLARED
                    and d.name not in self._package_names
                    and not any(
                        existing is not d
    ...
    def lower_type(self, d: TypeSpec) -> None:
        ...
        if isinstance(t, StructType):
            ...
            if fields:
                ...
                self.emit("def __init__(self, " + ", ".join(_ctor_name(x) + "=None" for x in fields) + ", **_go_fields):")
                self.indent += 1
                for n in fields:
                    z = self._zero_value_expr(field_types.get(n))
                    
                    if keyword.iskeyword(n) or n in {"self", "True", "False", "None"}:
                        self.emit(f"if {_ctor_name(n)} is None and {n!r} in _go_fields: {_ctor_name(n)} = _go_fields[{n!r}]")
            ...
            self.emit(f"class {py_ident(d.name)}(_go_builtins.list):")
            self.indent += 1
            self.emit("def __init__(self, *values):")
            self.indent += 1
            self.emit("if len(values) == 1 and _go_builtins.isinstance(values[0], (_go_builtins.list, _go_builtins.tuple)): values = tuple(values[0])")
            ...
            if array_len is not None:
                ...
                self.emit("_vals = _go_builtins.list(values)")
                self.emit("if _go_builtins.len(_vals) < _n: _vals.extend([0] * (_n - _go_builtins.len(_vals)))")
                self.emit("elif _go_builtins.len(_vals) > _n: _vals = _vals[:_n]")
            ...
            if isinstance(t, Expr) and t.kind == "type_name":
                ...
                if base:
                    ...
                    self.indent -= 1
                    return
                # Named Go scalar types can have non-predeclared underlying
                # types (notably cgo-generated _C_int/_C_size_t types).  Do
                # not collapse these to an assignment such as
                # `bufferKind = _C_int`: Go gives bufferKind its own method
                # namespace.  A Python subclass supplies that namespace while
                # retaining the underlying scalar behavior.
                try:
                    underlying = self.type_expr(t)
                except Exception:
                    underlying = None
                if underlying and underlying not in {py_ident(d.name), "None"}:
                    self.emit(f"class {py_ident(d.name)}({underlying}):")
                    self.indent += 1
                    self.emit("pass")
                    self.indent -= 1
                    return
    ...
    def _zero_value_expr(self, type_expr: Any) -> str:
        ...
        if isinstance(type_expr, Expr):
            ...
            if type_expr.kind == "array":
                ...
                elem = type_expr.children[0] if type_expr.children else None
                zero = "None"
                if isinstance(elem, Expr) and elem.kind == "type_name":
                    name = str(elem.value)
                    if name in {"bool"}: zero = "False"
                    elif name in {"float32", "float64"}: zero = "0.0"
                    elif name in {"complex64", "complex128"}: zero = "0j"
                    elif name in {"string"}: zero = "''"
                    elif name in {"byte", "rune", "int", "int8", "int16", "int32", "int64", "uint", "uint8", "uint16", "uint32", "uint64", "uintptr"}: zero = "0"
                if isinstance(length, Expr):
                    return f"[{zero}] * int({self.expr(length)})"
                try:
                    return f"[{zero}] * {int(str(length), 0)}"
                except (TypeError, ValueError):
                    return "[]"
    ...
    def lower_for(self, s: ForStmt) -> None:
        if s.range_expr is not None:
            ...
            if len(names) >= 2:
                ...
                self.emit(f"for {target} in _go_range_keys({source}):")
            self.indent += 1
            self._loop_depth += 1
            self._break_depth += 1
            ...
            self.indent -= 1
            return
        if s.cond is not None and (s.init is not None or s.post is not None):
            if s.init is not None:
    ...
    def expr(self, x: Any) -> str:
        if isinstance(x, Expr):
            ...
            if k == "name":
                ...
                name = str(x.value)
                if "." in name:
                    return ".".join(py_ident(part) for part in name.split("."))
            ...
            if k == "type_name":
                ...
                if "." in name:
                    parts = name.split(".")
                    return ".".join(py_ident(part) for part in parts)
                return py_ident(name)
            ...
            if k == "binary":
                ...
                # Large generated Go tables (notably net/http's bundled
                # IDNA data) contain thousands of adjacent string literals
                # joined with `+`.  Recursively parenthesizing every binary
                # node exceeds CPython's parser nesting limit.  Flatten only
                # the same associative operator; Python preserves the Go
                # left-to-right value for these concatenations.
                if x.value == "+":
                    parts = []
                    def _collect(node):
                        if isinstance(node, Expr) and node.kind == "binary" and node.value == "+":
                            _collect(node.children[0])
                            _collect(node.children[1])
                        else:
                            parts.append(self.expr(node))
                    _collect(x)
                    return "(" + " + ".join(parts) + ")"
            ...
            if k == "call":
                ...
                if isinstance(callee, Expr) and callee.kind == "name":
                    ...
                    if (callee.value == "make" and raw_args
                            and isinstance(raw_args[0], Expr) and raw_args[0].kind == "chan"):
                        self.require("runtime")
                ...
                # Go type assertions/conversions used for interface compile-time
                # checks commonly look like `Interface((*concrete)(nil))`.
                # Python has no corresponding static interface conversion; retain
                # the underlying value rather than attempting to call the
                # interface placeholder as a constructor.
                if (isinstance(callee, Expr) and callee.kind in ("type_name", "name")
                        and len(raw_args) == 1
                        and isinstance(raw_args[0], Expr)
                        and raw_args[0].kind == "unary" and raw_args[0].value in ("*", "&")
                        and isinstance(raw_args[0].children[0], Expr)
                        and raw_args[0].children[0].kind == "composite"):
                    return self.expr(raw_args[0].children[0])
            ...
            if k == "composite":
                base = x.value
                # Fixed-size array literals such as [4]byte{} are values,
                # not empty Python lists.  The zero elements must exist; an
                # empty list later reaches scalar conversions as None/out of
                # range.  This is especially important in net/netip, whose
                # package initialization constructs IPv4/IPv6 constants from
                # zero-valued arrays.
                if isinstance(base, Expr) and base.kind == "array":
                    length = None
                    if isinstance(base.value, Expr) and base.value.kind == "literal":
                        try:
                            length = int(str(base.value.value), 0)
                        except (TypeError, ValueError):
                            length = None
                    vals = [self.expr(a.children[1]) if isinstance(a, Expr) and a.kind == "keyed" else self.expr(a) for a in x.children]
                    if length is not None:
                        # Go's zero value for the common scalar array element
                        # types is directly representable in Python.
                        elem = base.children[0] if base.children else None
                        zero = "0"
                        if isinstance(elem, Expr) and elem.kind == "type_name" and str(elem.value) in {"float32", "float64"}:
                            zero = "0.0"
                        if not vals:
                            return f"[{zero}] * {length}"
                        return "[" + ", ".join(vals) + "] + [" + zero + "] * max(0, " + str(length) + " - " + str(len(vals)) + ")"
                    return "[" + ", ".join(vals) + "]"
                if isinstance(base, StructType):
                    ...
                    for a in x.children:
                        if isinstance(a, Expr) and a.kind == "keyed":
                            key = py_ident(a.children[0].value) if isinstance(a.children[0], Expr) and a.children[0].kind == "name" else self.expr(a.children[0])
...
# Resolver caches.  Package-name and module-root discovery sit on the hot path
# during large package translations; never invoke the Go toolchain just to
# answer a filesystem question.
_GO_MODULE_ROOTS_CACHE: Dict[tuple[str, tuple[str, ...]], List[Path]] = {}
_GO_MOD_REQUIREMENTS_CACHE: Dict[tuple[str, ...], List[tuple[str, str]]] = {}
_GO_MODULE_CACHE_ROOT: Optional[Path] = None
_GO_PACKAGE_NAME_CACHE: Dict[tuple[str, tuple[str, ...]], Optional[str]] = {}
...
def _go_module_escape_path(path: str) -> str:
    """Encode a module path using the Go module-cache escaping convention."""
    out = []
    for ch in path:
        if ch == "!":
            out.append("!!")
...
def _go_module_roots(import_path: str, search_paths: Optional[Sequence[str]] = None) -> List[Path]:
    """Map an import path to checked-out Go module roots, with memoization."""
    roots = tuple(os.path.abspath(str(p)) for p in (search_paths or ()))
    key = (import_path, roots)
    cached = _GO_MODULE_ROOTS_CACHE.get(key)
    if cached is not None:
        return cached
    scan_roots = list(roots)
    cwd = os.path.abspath(os.getcwd())
    if cwd not in scan_roots:
        scan_roots.append(cwd)
    result: List[Path] = []
    seen: Set[str] = set()
    for root in scan_roots:
        ...
        if mod.is_file():
            mods.append(mod)
        # Only shallow nested modules are relevant to the Moby layout.
        mods.extend(base.glob("*/go.mod"))
        mods.extend(base.glob("*/*/go.mod"))
        for modfile in mods:
            try:
            ...
            if import_path != module and not import_path.startswith(module + "/"):
                continue
            base_module = modfile.parent
            suffix = import_path[len(module):].lstrip("/")
            candidate = base_module / suffix if suffix else base_module
            normalized = os.path.normcase(os.path.abspath(str(candidate)))
            if normalized not in seen:
                seen.add(normalized)
                result.append(candidate)
    _GO_MODULE_ROOTS_CACHE[key] = result
    return result

def _go_mod_requirements(search_paths: Optional[Sequence[str]] = None) -> List[tuple[str, str]]:
    """Return module requirements visible from the supplied source roots.

    This is intentionally a lightweight go.mod reader.  It does not resolve
    versions or execute Go tooling; it only exposes the exact module/version
    pairs already declared by a checked-out module.
    """
    roots = tuple(os.path.abspath(str(p)) for p in (search_paths or ()))
    cached = _GO_MOD_REQUIREMENTS_CACHE.get(roots)
    if cached is not None:
        return cached
    scan_roots = list(roots)
    cwd = os.path.abspath(os.getcwd())
    ...
    for root in roots:
        ...
        for raw in text.splitlines():
            ...
            if line.startswith("require "):
                parts = line.split()
                if len(parts) >= 3:
                    requirements.append((parts[1], parts[2]))
    # Preserve first declaration and avoid duplicate cache probes.
    unique = []
    ...
    for req in requirements:
        if req not in seen_req:
            seen_req.add(req)
            unique.append(req)
    _GO_MOD_REQUIREMENTS_CACHE[roots] = unique
    return unique


def _go_module_cache_candidates(import_path: str, search_paths: Optional[Sequence[str]] = None) -> List[Path]:
    """Find an imported package in the Go module cache without spawning `go`."""
    global _GO_MODULE_CACHE_ROOT
    if _GO_MODULE_CACHE_ROOT is None:
        configured = os.environ.get("GOMODCACHE")
        if configured:
            _GO_MODULE_CACHE_ROOT = Path(configured)
        else:
            # Go's default is GOPATH/pkg/mod.  Deriving this locally is much
            # cheaper and safer than running `go env` for every import.
            gopath = os.environ.get("GOPATH")
            if gopath:
                _GO_MODULE_CACHE_ROOT = Path(gopath.split(os.pathsep)[0]) / "pkg" / "mod"
            else:
                _GO_MODULE_CACHE_ROOT = Path.home() / "go" / "pkg" / "mod"
    if not _GO_MODULE_CACHE_ROOT.is_dir():
        return []
    candidates: List[Path] = []
    for module, version in _go_mod_requirements(search_paths):
        if import_path != module and not import_path.startswith(module + "/"):
            continue
        suffix = import_path[len(module):].lstrip("/")
        root = _GO_MODULE_CACHE_ROOT / (_go_module_escape_path(module) + "@" + version)
...
def _go_disk_candidates(import_path: str, search_paths: Optional[Sequence[str]] = None) -> List[Path]:
    ...
    # Prefer a checked-out module root when the import belongs to that
    # module.  This handles module paths such as github.com/moby/moby/v2
    # without requiring callers to monkey-patch the resolver.
    candidates.extend(_go_module_roots(import_path, search_paths))
    roots = list(search_paths or ())
    for env_name in ("GO_PATH", "GOPATH", "GOROOT"):
        value = os.environ.get(env_name)
        if value:
            roots.extend(value.split(os.pathsep))
    # GOROOT is commonly unset even though `go` is installed.  Derive it
    # directly from the executable path instead of spawning `go env` for every
    # import.  This is important for standard-library packages such as
    # net/netip, maps, iter, and unique.
    if not os.environ.get("GOROOT"):
        go_exe = shutil.which("go")
        if go_exe:
            roots.append(str(Path(go_exe).resolve().parent.parent))
    roots.append(os.getcwd())
    # A Go module's external dependencies normally live in GOMODCACHE rather
    # than under GOPATH/src.  Add those exact versioned module paths after the
    # ordinary source roots so local/vendor copies retain precedence.
    candidates.extend(_go_module_cache_candidates(import_path, search_paths))
    seen: Set[str] = set()
    for root in roots:
        for candidate in (
            Path(root) / import_path,
            Path(root) / "src" / import_path,
            Path(root) / "external_go_imports" / import_path,
        ):
...
def _go_source_allowed(path: Path) -> bool:
    # Build eligibility depends on the active GOOS/GOARCH.  Key the cache by
    # target as well as pathname so a long-lived translator cannot reuse a
    # decision made for a different target.
    key = (str(path), _go_build_env()[:2])
...
def _go_disk_package_name(import_path: str, search_paths: Optional[Sequence[str]]) -> Optional[str]:
    ...
    roots = tuple(os.path.abspath(str(p)) for p in (search_paths or ()))
    key = (import_path, roots)
    if key in _GO_PACKAGE_NAME_CACHE:
        return _GO_PACKAGE_NAME_CACHE[key]
    result = None
    for candidate in _go_disk_candidates(import_path, search_paths):
        ...
        for go_file in sorted(candidate.glob("*.go")):
            ...
            if match:
                ...
                _GO_PACKAGE_NAME_CACHE[key] = result
                return result
    _GO_PACKAGE_NAME_CACHE[key] = result
    return result
...
def load_go_package(
    ...
    *,
    ...
    if import_path in _GO_PACKAGE_CACHE:
        ...
        # Translation-only graph walks may install a deferred external
        # package before execution.  If real source becomes available,
        # execution must resolve that source instead of permanently reusing
        # the deferred placeholder.
        if execute and getattr(pkg, "__go_external__", False):
            _GO_PACKAGE_CACHE.pop(import_path, None)
            cache.pop(import_path, None)
        else:
    ...
    if import_path == "reflect":
        ...
        _kind_names = {
            ...
            "Float64": 15, "Complex64": 16, "Complex128": 17, "Array": 17,
            "Chan": 18, "Func": 19, "Interface": 20, "Map": 21,
            "Pointer": 22, "Slice": 23, "String": 24, "Struct": 25,
            "UnsafePointer": 26,
        }
    ...
    # Docker's image-spec package is a small schema layer.  The daemon path
    # needs its Go structs, not an independent Python implementation of the
    # OCI image format, so expose struct-compatible host objects here.
    if import_path == "github.com/moby/docker-image-spec/specs-go/v1":
        class _SpecStruct:
        ...
        class _DockerSpecPackage(GoPackage):
            def __getattr__(self, name):
                if name in self:
                    return self[name]
                cls = type(name, (_SpecStruct,), {})
                self[name] = cls
                return cls

        ns = _DockerSpecPackage({"__name__": import_path})
        for _name in ("DockerOCIImage", "DockerOCIImageConfig",
                      "DockerOCIImageConfigExt", "HealthcheckConfig"):
            ns[_name] = type(_name, (_SpecStruct,), {})
        _GO_PACKAGE_CACHE[import_path] = ns
        cache[import_path] = ns
        return ns

    # golang.org/x/sys/unix is an OS syscall boundary.  The Moby daemon's
    # translated Python runtime should bind the small set of libc/Python OS
    # operations it actually reaches instead of requiring the entire generated
    # x/sys/unix source tree.
    if import_path == "golang.org/x/sys/unix":
        import os as _go_unix_os
        import socket as _go_unix_socket
        import fcntl as _go_unix_fcntl

        class _UnixPackage(GoPackage):
            def __getattr__(self, name):
                if name in self:
                    return self[name]
                for _mod in (_go_unix_os, _go_unix_socket, _go_unix_fcntl):
                    if hasattr(_mod, name):
                        value = getattr(_mod, name)
                        self[name] = value
                        return value
                _known = {
                    "Getpid": _go_unix_os.getpid,
                    "Getppid": _go_unix_os.getppid,
                    "Getuid": _go_unix_os.getuid,
                    "Geteuid": _go_unix_os.geteuid,
                    "Getgid": _go_unix_os.getgid,
                    "Getegid": _go_unix_os.getegid,
                }
                if name in _known:
                    self[name] = _known[name]
                    return _known[name]
                if name.isupper() or name.startswith(("AF_", "SO_", "SOL_", "O_", "CLONE_", "MS_", "MNT_", "IFF_", "E", "SIG")):
                    self[name] = 0
                    return 0
                def _unsupported(*args, **kwargs):
                    raise NotImplementedError(f"golang.org/x/sys/unix.{name} is not host-bound")
                self[name] = _unsupported
                return _unsupported

        ns = _UnixPackage({"__name__": import_path})
        _GO_PACKAGE_CACHE[import_path] = ns
        cache[import_path] = ns
        return ns

    # internal/bytealg is compiler/runtime glue used by net/netip and other
    # standard-library packages. Python strings already provide the required
    # byte-search primitive for the translated code.
    if import_path == "internal/bytealg":
        def _IndexByteString(s, c):
            if isinstance(c, int):
                c = chr(c)
            return str(s).find(str(c))
        ns = GoPackage({"__name__": "internal/bytealg",
                        "IndexByteString": _IndexByteString})
        _GO_PACKAGE_CACHE[import_path] = ns
        cache[import_path] = ns
        return ns

    # internal/godebug is used by time during package initialization.  The
    # translated runtime does not need the full Go godebug registry here; it
    # needs a Setting object with the same observable API.  Read GODEBUG once
    # per setting and otherwise use Go's empty/default value semantics.
    if import_path == "internal/godebug":
        import os as _go_host_os
        ...
        class _Setting:
            ...
            def Value(self):
                if self._value is None:
                    value = ""
                    for part in _go_host_os.environ.get("GODEBUG", "").split(","):
                        if "=" in part:
            ...
            def IncNonDefault(self):
                return None

        def _New(name):
            return _Setting(name)

        ns = GoPackage({"__name__": "internal.godebug",
                        "Setting": _Setting, "New": _New})
        _GO_PACKAGE_CACHE[import_path] = ns
        cache[import_path] = ns
        return ns

    # internal/reflectlite is compiler/runtime-facing code.  Its Go source
    # expects a real Go interface value containing an ABI type pointer; a plain
    # Python value cannot provide that layout.  Keep this boundary host-backed,
    # just as the public reflect package above, while preserving the API used
    # by Moby and the standard library.
    if import_path == "internal/reflectlite":
        class _GoInterfaceMarker:
            pass
        ...
        class _Type:
            ...
            def Comparable(self):
                k = self.Kind()
                return k not in (20, 21, 23)
            def Name(self): return ""
            def PkgPath(self): return ""
            def Size(self): return 0
            def Implements(self, other): return False
            def AssignableTo(self, other): return True
            def Elem(self):
                # A typed nil pointer such as (*error)(nil) is lowered to
                # Python None, so the host boundary cannot recover its static
                # pointer type from the value alone.  Treat nil as the common
                # typed-pointer case required by errors/wrap.go.
                if self.value is None:
                    return _Type(_GoInterfaceMarker())
                raise TypeError("reflect: Elem of non-pointer type")
        ...
        def _kind(value):
            ...
            if isinstance(value, _GoInterfaceMarker): return 20
            if isinstance(value, bool): return 1
            if isinstance(value, int): return 2
            if isinstance(value, float): return 14
            if isinstance(value, complex): return 17
            if isinstance(value, str): return 24
            if isinstance(value, (bytes, bytearray, list, tuple)): return 23
            if isinstance(value, dict): return 21
            if callable(value): return 19
            return 25

        ns = GoPackage({
            "__name__": "internal.reflectlite",
            "Type": _Type, "Value": _Value,
            "TypeOf": lambda value: _Type(value),
            "ValueOf": lambda value: _Value(value),
        })
        ns.update({
            "Invalid": 0, "Bool": 1, "Int": 2, "Int8": 3, "Int16": 4,
            "Int32": 5, "Int64": 6, "Uint": 7, "Uint8": 8, "Uint16": 9,
    ...
    # Go 1.21+ maps helpers are generic wrappers around native map operations.
    # They are small enough to map directly to Python dictionaries/sequences,
    # avoiding recursive translation of the standard-library implementation.
    if import_path == "maps":
        def Clone(m):
        ...
        def EqualFunc(a, b, f):
            ...
            if len(a) != len(b) or set(a) != set(b):
                return False
            return all(f(k, a[k], b[k]) for k in a)
        def Keys(m):
            return iter(()) if m is None else iter(m.keys())
        def Values(m):
            return iter(()) if m is None else iter(m.values())
        def Delete(m, key):
            if m is not None:
                m.pop(key, None)
        ns = GoPackage({"__name__": "maps", "Clone": Clone, "Copy": Copy,
                        "DeleteFunc": DeleteFunc, "Equal": Equal,
                        "EqualFunc": EqualFunc, "Keys": Keys, "Values": Values,
                        "Delete": Delete})
        _GO_PACKAGE_CACHE[import_path] = ns
        cache[import_path] = ns
        return ns

    # Go 1.23+ unique handles are implemented by the runtime using weak
    # references and type-specific maps.  For the pure-Python translation
    # boundary we only need the public value/identity semantics used by Moby:
    # Handle[T], Make(T), and Handle.Value().  Keeping the value strongly
    # referenced is intentional here; it avoids pulling in runtime/internal/*
    # solely to emulate the implementation machinery.
    if import_path == "unique":
        class Handle:
            ...
            def Value(self):
                if not self._has_value:
                    raise ValueError("zero unique.Handle has no value")
                return self._value

            def __eq__(self, other):
                if not isinstance(other, Handle):
                    return NotImplemented
                if not self._has_value or not other._has_value:
                    return self._has_value == other._has_value
                return self._value == other._value

            def __hash__(self):
                return hash((self._has_value, self._value))

        def Make(value):
            return Handle(value)

        ns = GoPackage({"__name__": "unique", "Handle": Handle, "Make": Make})
        _GO_PACKAGE_CACHE[import_path] = ns
        cache[import_path] = ns
        return ns

    # Go 1.23+ iterators are language-adjacent standard-library support.
    # Translating iter.go recursively pulls in the entire runtime, so expose
    # the same public Seq/Seq2/Pull contract at this boundary.
    if import_path == "iter":
        class Seq:
        ...
        def _Pull(seq):
            values = []
            stopped = False
            def yield_value(*args):
                if stopped: return False
                values.append(args); return True
            seq(yield_value)
            index = 0
            def next_value():
                nonlocal index
                if index >= len(values): return None, False
                value = values[index]; index += 1
                return (value[0] if len(value) == 1 else tuple(value)), True
            def stop():
                nonlocal stopped
                stopped = True
            return next_value, stop
        ns = GoPackage({"__name__": "iter", "Seq": Seq, "Seq2": Seq2, "Pull": _Pull})
        _GO_PACKAGE_CACHE[import_path] = ns
        cache[import_path] = ns
        return ns
    ...
    # github.com/moby/term is an external module used by dockerd.  The
    # current Moby checkout does not vendor it, and an offline execution
    # environment cannot populate GOMODCACHE.  Keep the package boundary
    # explicit and host-backed instead of fabricating Go source files.
    if import_path == "github.com/moby/term":
        ...
        def _to_bytes(keys):
            ...
            for key in str(keys).split(","):
                ...
                if key == "DEL":
                    ...
                    continue
                try:
                    out.append(_ascii.index(key))
                except ValueError as exc:
                    raise ValueError(f"Unknown character: '{key}'") from exc
            return bytes(out)
        ...
        def _get_fd_info(value):
            fd = _fileno(value)
            try:
                return fd, bool(fd >= 0 and _os.isatty(fd))
            except Exception:
                return fd, False
        ...
        def _get_winsize(fd):
            try:
                ...
                return _Winsize(size.lines, size.columns), None
            except Exception as exc:
                return None, exc

        def _std_streams():
            return _sys.stdin, _sys.stdout, _sys.stderr

        class _EscapeProxy:
            def __init__(self, reader, escape_keys):
                self.reader = reader
                self.escape_keys = bytes(escape_keys or b"")
                self._pending = bytearray()

            def Read(self, buf):
                # Go's proxy is an io.Reader.  Python callers may expose either
                # read(n) or Read(bytearray) semantics; support both.
                if self._pending:
                    ...
                    if read is None:
                        ...
                        if isinstance(data, str):
                            data = data.encode()
                if self.escape_keys:
                    pos = data.find(self.escape_keys)
                    if pos >= 0:
                        before = data[:pos]
                        after = data[pos + len(self.escape_keys):]
                        self._pending.extend(after)
                        try:
                            buf[:len(before)] = before
                        except Exception:
                            pass
                        return len(before), _EscapeError()
                try:
                    buf[:len(data)] = data
                except Exception:
                    pass
            ...
            def read(self, n=-1):
                if hasattr(self.reader, "read"):
                    ...
                    data = bytes(self._pending)
                    self._pending.clear()
                if self.escape_keys and data:
                    pos = data.find(self.escape_keys)
                    if pos >= 0:
                        self._pending.extend(data[pos + len(self.escape_keys):])
                        raise _EscapeError()
                return data

        ns = GoPackage({
            "__name__": import_path,
            "ASCII": _ascii,
            "ToBytes": _to_bytes,
            "EscapeError": _EscapeError,
            "Winsize": _Winsize,
            "StdStreams": _std_streams,
            "GetFdInfo": _get_fd_info,
            "GetWinsize": _get_winsize,
            "NewEscapeProxy": lambda reader, keys: _EscapeProxy(reader, keys),
            "IsTerminal": lambda fd: bool(_get_fd_info(fd)[1]),
        })
        _GO_PACKAGE_CACHE[import_path] = ns
        cache[import_path] = ns
        return ns
        ...
        if candidate.is_dir():
            for py_file in (candidate / "__init__.py", candidate / f"{candidate.name}.py"):
                if py_file.exists():
                    ...
                    cache[import_path] = ns
                    return ns
            go_files = sorted(p for p in candidate.glob("*.go") if _go_source_allowed(p) and p.stat().st_size > 0)
            if go_files:
                ...
                for go_file in go_files:
                    try:
                        ...
                        raise type(exc)(f"{go_file}: {exc}").with_traceback(exc.__traceback__) from exc
                ...
                if not execute:
                    ...
                    for go_file, generated in generated_files:
                        compile(generated, str(go_file), "exec")
                    child_imports = set()
                    for go_file in go_files:
                        tree = parse_go(go_file.read_text(encoding="utf-8"))
                        child_imports.update(imp.path for imp in tree.imports)
                    # Translation-only graph traversal is intentionally local
                    # to the requested checked-out module.  Standard-library
                    # and third-party packages remain explicit external
                    # boundaries; translating enormous generated stdlib files
                    # (for example net/http/h2_bundle.go and x/net/idna tables)
                    # adds little information about the Moby package graph and
                    # can exceed Python parser limits.  Execution mode still
                    # resolves those packages normally.
                    local_modules = []
                    for root in (search_paths or ()):
                        mod = Path(root) / "go.mod"
                        if mod.is_file():
                            try:
                                text = mod.read_text(encoding="utf-8")
                                m = re.search(r"(?m)^\s*module\s+(\S+)\s*$", text)
                                if m:
                                    local_modules.append(m.group(1))
                            except OSError:
                                pass
                    for child in sorted(child_imports):
                        if child in {"C", "unsafe"}:
                            continue
                        if any(child == mod or child.startswith(mod + "/") for mod in local_modules):
                            load_go_package(child, cache, search_paths=search_paths, execute=False)
                        else:
                            # Materialize a deferred external boundary without
                            # descending into its source tree.
                            class _ExternalPackage(GoPackage):
                                def __init__(self, path):
                                    super().__init__({"__name__": path, "__go_external__": True})
                                    self.__go_import_path__ = path
                                def __getattr__(self, name):
                                    raise ImportError(
                                        f"external Go package {self.__go_import_path__!r} is unresolved; "
                                        f"member {name!r} requires its module source at execution time"
                                    )
                            cache[child] = _ExternalPackage(child)
                            _GO_PACKAGE_CACHE[child] = cache[child]
                    return ns
    ...
    # Translation-only mode deliberately permits unresolved external modules.
    # Their source is not required to parse/compile the local package graph;
    # execution mode still raises so missing runtime dependencies cannot be
    ...
    if not execute:
        ...
        ns = _ExternalPackage(import_path)
        _GO_PACKAGE_CACHE[import_path] = ns
        cache[import_path] = ns
        return ns
