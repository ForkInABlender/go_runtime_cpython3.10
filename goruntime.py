...
def select(*cases: _Case) -> Any:
    """Select one ready communication and return its callback result."""
    cases = list(cases)
    if not cases:
        return
    ...
    if defaults:
        return defaults[0].callback()  # type: ignore[misc]
